...
class GameMaster {
  ...
  bindEvents() {
    ...
    //   this.vacuum.x = e.pageX; this.vacuum.y = e.pageY;
    // });
  }
  onMousedown(e) {
  ...
  moveBoids() {
    ...
    //   this.cohesion(boid);
      if(this.separation(boid)) return;
      // this.alignment(i);
    ...
    if(this.boids.size < MAX_N_BOIDS && Math.random() < 0.7) this.boids.add({
  ...
  cohesion(boid) {
    ...
    for(const another of this.boids) {
      if( boid === another ) {
        continue;
      }
      center.x += another.x;
    ...
    center.x /= this.boids.size - 1;
    center.y /= this.boids.size - 1;

    boid.vx += (center.x - boid.x) / 100;
    boid.vy += (center.y - boid.y) / 100;
  }
  separation(boid) {
    const distance = this.getDistance( this.vacuum, boid );
    if(distance < 10) {
      this.boids.delete(boid)
      // this.boids.splice(index, 1); // 削除
      this.vacuum.points++;
      return true;
    }
    if( distance < 100 ) {
        boid.vx += (this.vacuum.x - boid.x);
        boid.vy += (this.vacuum.y - boid.y);
    }
    for(const another of this.boids) {
      if( boid === another ) {
        continue;
      }
      const distance = this.getDistance(boid, another);

      if( distance < 10 ) {
        boid.vx -= another.x - boid.x;
        boid.vy -= another.y - boid.y;
      }
    }
    return false;
  }
